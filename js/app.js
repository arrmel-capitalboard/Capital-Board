...
// benchmark, projections, earnings, recap, alertes) n'y figurent
      ...
      tr.innerHTML = `
        <td>
          <div class="ticker-cell">
            ...
            <div>
              <div class="ticker-name" title="${_attr(row.name || row.ticker)}">${row.name || row.ticker}<span class="${_isEtfRow(row) ? 'badge-etf' : 'badge-action'}">${_isEtfRow(row) ? 'ETF' : 'ACTION'}</span></div>
...
function startKpisAutoScroll(elId) {
  ...
  if (!el._kpisState) {
    ...
    const resume = () => { st.pos = el.scrollLeft; st.paused = false; };
  ...
  const st = el._kpisState;
  st.pos = el.scrollLeft;
  let last = 0;
  const step = (ts) => {
    el._kpisRaf = requestAnimationFrame(step);
    if (!last) last = ts;
    const dt = Math.min(ts - last, 100); // onglet en arrière-plan : pas de saut
    last = ts;
    if (st.paused) return;
...
function initChartExpandButtons() {
  ['chart-portfolio', 'chart-benchmark', 'chart-projections'].forEach(_addExpandBtn);
}
...
const _PROFIL_LIBELLES = {
  ...
  source: { bouche: 'Bouche-à-oreille', reseaux: 'Réseaux sociaux', recherche: 'Recherche web', ia: 'Une IA', discord: 'Discord', autre: 'Autre' },
...
// Icônes de concept, pour tout ce qui n'est pas une marque.
//
// Un loyer, un plein d'essence ou une facture d'électricité n'ont pas de logo :
// ils avaient donc droit à deux initiales, ce qui ne dit rien. Le catalogue de
// marchands ne peut pas les couvrir — ce ne sont pas des enseignes, ce sont des
// natures de dépense.
//
// L'ordre compte : le premier motif qui tombe gagne. Les cas précis passent donc
// avant les génériques — « assurance auto » doit donner un bouclier, pas une
// voiture.
const DEP_ICONES = [
  { re: /\b(loyer|location|locatif|appartement|appart|studio|colocation|coloc|logement)\b/,
    p: '<path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/><path d="M10 21v-6h4v6"/>' },
  { re: /\b(credit|pret|prets|emprunt|mensualite|mensualites|remboursement)\b/,
    p: '<path d="M3 10h18L12 3 3 10z"/><path d="M5 10v8M9.5 10v8M14.5 10v8M19 10v8"/><path d="M3 21h18"/>' },
  { re: /\b(assurance|assurances|mutuelle|complementaire|prevoyance|habitation|rc)\b/,
    p: '<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>' },
  { re: /\b(electricite|elec|courant|edf|kwh)\b/,
    p: '<path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/>' },
  { re: /\b(gaz|chauffage|fioul|granules|pellets)\b/,
    p: '<path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5S5 13 5 15a7 7 0 0 0 7 7z"/>' },
  { re: /\b(eau|assainissement|veolia|suez)\b/,
    p: '<path d="M12 2.7 6.5 9.5a7 7 0 1 0 11 0L12 2.7z"/>' },
  { re: /\b(internet|box|fibre|adsl|wifi)\b/,
    p: '<path d="M5 12.6a11 11 0 0 1 14 0"/><path d="M1.5 9a16 16 0 0 1 21 0"/><path d="M8.5 16.1a6 6 0 0 1 7 0"/><line x1="12" y1="20" x2="12.01" y2="20"/>' },
  { re: /\b(forfait|mobile|telephone|tel|portable|sim)\b/,
    p: '<rect x="5" y="2" width="14" height="20" rx="2.5"/><line x1="12" y1="18" x2="12.01" y2="18"/>' },
  { re: /\b(sante|medecin|pharmacie|dentiste|opticien|kine|laboratoire|consultation)\b/,
    p: '<rect x="3" y="3" width="18" height="18" rx="4"/><path d="M12 8v8M8 12h8"/>' },
  { re: /\b(sport|salle|gym|muscu|fitness|piscine|club)\b/,
    p: '<path d="M6.5 6.5v11M17.5 6.5v11"/><path d="M3 9.5v5M21 9.5v5"/><path d="M6.5 12h11"/>' },
  { re: /\b(essence|carburant|gasoil|gazole|diesel|sp95|sp98|plein|station)\b/,
    p: '<path d="M4 22V4a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v18"/><path d="M2 22h14"/><path d="M6 8h6"/><path d="M14 10h3a2 2 0 0 1 2 2v5.5a1.5 1.5 0 0 0 3 0V9l-3-3"/>' },
  { re: /\b(peage|autoroute|vinci|sanef|aprr)\b/,
    p: '<path d="M4 22 8 2M20 22 16 2"/><path d="M12 6v3M12 12v3M12 18v3"/>' },
  { re: /\b(parking|stationnement|garage|box auto|indigo|effia)\b/,
    p: '<rect x="3" y="3" width="18" height="18" rx="4"/><path d="M9.5 17V7h3.2a3 3 0 0 1 0 6H9.5"/>' },
  { re: /\b(train|metro|bus|tram|navigo|abonnement transport|sncf|ratp|tcl|tan)\b/,
    p: '<rect x="4" y="3" width="16" height="13" rx="2.5"/><line x1="4" y1="11" x2="20" y2="11"/><circle cx="8.5" cy="13.5" r=".9"/><circle cx="15.5" cy="13.5" r=".9"/><path d="m7 19-2 3M17 19l2 3"/>' },
  { re: /\b(voiture|auto|vehicule|entretien auto|revision|pneus)\b/,
    p: '<path d="M5 17H3v-5l2-5h14l2 5v5h-2"/><circle cx="7.5" cy="17" r="2"/><circle cx="16.5" cy="17" r="2"/><path d="M9.5 17h5"/>' },
  { re: /\b(courses|alimentation|supermarche|epicerie|drive|panier)\b/,
    p: '<circle cx="9" cy="21" r="1.2"/><circle cx="19" cy="21" r="1.2"/><path d="M2 2h2.2l2.7 12.4a2 2 0 0 0 2 1.6h9.4a2 2 0 0 0 2-1.6L22 7H5.2"/>' },
  { re: /\b(restaurant|resto|midi|dejeuner|diner|brasserie|cantine)\b/,
    p: '<path d="M4 2v7a2 2 0 0 0 2 2 2 2 0 0 0 2-2V2"/><path d="M6 11v11"/><path d="M17 2c-1.7 1.6-2.5 3.7-2.5 6 0 1.7.8 2.6 2.5 3v11"/>' },
  { re: /\b(cafe|bar|the|boulangerie)\b/,
    p: '<path d="M17 8h1a4 4 0 1 1 0 8h-1"/><path d="M3 8h14v9a4 4 0 0 1-4 4H7a4 4 0 0 1-4-4z"/><line x1="6" y1="2" x2="6" y2="5"/><line x1="10" y1="2" x2="10" y2="5"/><line x1="14" y1="2" x2="14" y2="5"/>' },
  { re: /\b(impot|impots|taxe|taxes|fisc|urssaf|cotisation|cotisations|fonciere|habitation taxe)\b/,
    p: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="13" y2="17"/>' },
  { re: /\b(salaire|paie|paye|prime|remuneration|honoraires|facture client)\b/,
    p: '<path d="M19 5H5a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2z"/><path d="M3 10h18"/><circle cx="17" cy="14.5" r="1.4"/>' },
  { re: /\b(epargne|livret|virement epargne|mise de cote|pel|lep)\b/,
    p: '<ellipse cx="12" cy="6" rx="8" ry="3"/><path d="M4 6v6c0 1.7 3.6 3 8 3s8-1.3 8-3V6"/><path d="M4 12v6c0 1.7 3.6 3 8 3s8-1.3 8-3v-6"/>' },
  { re: /\b(animal|animaux|chien|chat|veterinaire|veto|croquettes)\b/,
    p: '<circle cx="6.5" cy="9.5" r="1.9"/><circle cx="11" cy="6.5" r="1.9"/><circle cx="16" cy="8" r="1.9"/><circle cx="19.5" cy="12.5" r="1.7"/><path d="M8.6 15.4c1.6-2 4.6-2.3 6.4-.7 1 .9 1.4 2.2 1 3.4-.5 1.6-2.3 2.4-4.1 2.4s-3.6-.8-4.1-2.4c-.3-.9 0-1.9.8-2.7z"/>' },
  { re: /\b(creche|garderie|nounou|ecole|scolarite|periscolaire|enfant|enfants)\b/,
    p: '<path d="M6.5 8a5.5 5.5 0 0 1 11 0"/><rect x="3" y="8" width="18" height="13" rx="3.5"/><path d="M9.5 21v-6h5v6"/>' },
  { re: /\b(coiffeur|coiffure|esthetique|barbier|manucure)\b/,
    p: '<circle cx="6" cy="6" r="2.6"/><circle cx="6" cy="18" r="2.6"/><line x1="20" y1="4" x2="8.2" y2="15.8"/><line x1="14.5" y1="14.5" x2="20" y2="20"/><line x1="8.2" y1="8.2" x2="12" y2="12"/>' },
  { re: /\b(menage|femme de menage|pressing|blanchisserie|laverie)\b/,
    p: '<rect x="4" y="2" width="16" height="20" rx="3"/><circle cx="12" cy="14" r="4.5"/><line x1="8" y1="6" x2="8.01" y2="6"/><line x1="12" y1="6" x2="12.01" y2="6"/>' },
  { re: /\b(vacances|voyage|hotel|billet|avion|sejour)\b/,
    p: '<path d="M17.8 19.2 16 11l3.5-3.5a2.1 2.1 0 0 0-3-3L13 8 4.8 6.2a1 1 0 0 0-.9 1.7l5.6 3.4-2.3 2.3-2.4-.5a1 1 0 0 0-.9 1.6l4.8 4.8a1 1 0 0 0 1.6-.9l-.5-2.4 2.3-2.3 3.4 5.6a1 1 0 0 0 1.7-.9z"/>' },
  { re: /\b(cadeau|cadeaux|anniversaire|noel)\b/,
    p: '<polyline points="20 12 20 22 4 22 4 12"/><rect x="2" y="7" width="20" height="5" rx="1"/><line x1="12" y1="22" x2="12" y2="7"/><path d="M12 7H7.5a2.5 2.5 0 0 1 0-5C11 2 12 7 12 7z"/><path d="M12 7h4.5a2.5 2.5 0 0 0 0-5C13 2 12 7 12 7z"/>' },
  { re: /\b(don|dons|association|caritatif|parrainage|soutien)\b/,
    p: '<path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1-1.1a5.5 5.5 0 0 0-7.8 7.8l8.8 8.7 8.8-8.7a5.5 5.5 0 0 0 0-7.8z"/>' },
  { re: /\b(jeu|jeux|console|gaming)\b/,
    p: '<rect x="2" y="6" width="20" height="12" rx="6"/><line x1="6.5" y1="12" x2="10.5" y2="12"/><line x1="8.5" y1="10" x2="8.5" y2="14"/><line x1="15" y1="12.5" x2="15.01" y2="12.5"/><line x1="18" y1="10.5" x2="18.01" y2="10.5"/>' },
  { re: /\b(presse|journal|magazine|livre|livres|abonnement presse)\b/,
    p: '<path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>' },
  { re: /\b(musique|concert|spectacle|cinema|theatre)\b/,
    p: '<path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/>' },
  { re: /\b(streaming|abonnement|souscription|licence|logiciel)\b/,
    p: '<rect x="2" y="4" width="20" height="16" rx="3.5"/><path d="m10 9 5 3-5 3z"/>' },
];

// Comme _depNorm, mais les mots restent séparés — indispensable pour que les
// limites de mot (\b) des motifs d'icônes veuillent dire quelque chose.
function _depMots(s) {
...
// Icône de concept pour un intitulé, ou null. Rendue à la couleur du poste.
function _depIcone(nom) {
  const m = _depMots(nom);
  if (!m) return null;
  const hit = DEP_ICONES.find(i => i.re.test(m));
  return hit ? hit.p : null;
}
function _depSvg(paths, taille) {
...
function _depChipHtml(e, c) {
  ...
  if (!dom) {
    // Pas de marque connue : une icône de concept si l'intitulé en évoque une
    // (loyer, essence, électricité…), les initiales sinon.
    const ico = _depIcone(e.nom);
    return '<div class="dep-chip" style="' + fond + '">' +
      (ico ? _depSvg(ico, 17) : init) + '</div>';
  }
...
function _depRenderLogo() {
  ...
  // Écrire « loyer » doit suffire à voir apparaître une maison : c'est le
  // retour immédiat qui dit que l'intitulé a été compris.
  const ico = _depIcone(nom);
  if (ico) box.innerHTML = _depSvg(ico, 22);
  else box.textContent = nom ? _depInitials(nom) : '?';
}
...
// Index de quinzaine d'une date dans son année : 0 pour le 1er-15 janvier,
// 1 pour le 16-31 janvier, jusqu'à 23.
function _livQIndex(d) { return d.getMonth() * 2 + (d.getDate() >= 16 ? 1 : 0); }

/**
 * Quinzaine à partir de laquelle une somme commence à produire.
 *
 * Règle des livrets : un versement ne rapporte qu'à compter du 1er ou du 16
 * qui suit. Déposer le 3 janvier fait courir les intérêts au 16 janvier ;
 ...
 *
 * Deux régimes, selon ce que le membre a saisi.
 *
 * Avec l'historique des versements, le calcul est juste : chaque somme compte
 * à partir de la quinzaine qui suit son dépôt. C'est ce qui fait qu'un même
 * solde rapporte très différemment selon qu'il est arrivé en janvier ou en
 * juin — et c'est ce que la version précédente ignorait, en supposant le solde
 * présent depuis le 1er janvier.
 *
 * Sans historique, on retombe sur cette hypothèse, et `exact` vaut false pour
...
window.livSetType = function(t) { _livType = t; _livRenderTypes(); _livRenderLogo(); livDatesSync(); livRecalc(); };

// ─── Dates : ouverture et fin se déduisent l'une de l'autre ───

let _livMvts = [];      // brouillon de saisie, confirmé à l'enregistrement

function _livAnnee(iso) { const n = Number(String(iso || '').slice(0, 4)); return n > 1900 ? n : null; }
// Décale une date ISO de n années, en gardant jour et mois.
function _livPlusAns(iso, n) {
  const p = String(iso || '').split('-');
  if (p.length < 3) return '';
  return String(Number(p[0]) + n) + '-' + p[1] + '-' + p[2];
}

/**
 * Remplit la date manquante à partir de l'autre, quand la loi la détermine.
 *
 * Le PEL a un terme fixe de quinze ans : les deux dates s'en déduisent l'une
 * de l'autre. Le Livret Jeune, non — il se ferme le 31 décembre de l'année des
 * 25 ans, et la date d'ouverture ne dit pas l'âge qu'on avait (on peut ouvrir
 * de 12 à 25 ans). Dans ce cas on ne devine pas : on explique la règle, et on
 * déduit l'année de naissance de la date de fin quand elle est posée.
 *
 * @param {string} source champ que l'utilisateur vient de modifier
 */
window.livDateChange = function(source) {
  const t   = _livType_(_livType);
  const ouv = _depVal('liv-f-ouverture');
  const fin = _depVal('liv-f-fin');
  if (t.duree) {
    if (source === 'ouverture' && ouv && !fin) _depSet('liv-f-fin', _livPlusAns(ouv, t.duree));
    if (source === 'fin' && fin && !ouv)       _depSet('liv-f-ouverture', _livPlusAns(fin, -t.duree));
  }
  livDatesSync();
  livRecalc();
};

function livDatesSync() {
  const t   = _livType_(_livType);
  const ouv = _depVal('liv-f-ouverture');
  const fin = _depVal('liv-f-fin');
  ...
  if (t.duree) {
    el.textContent = 'Terme légal de ' + t.duree + ' ans : renseignez une date, l\'autre se remplit toute seule.';
  } else if (_livType === 'jeune') {
    const an = _livAnnee(fin);
    el.textContent = an
      ? 'Un Livret Jeune se ferme le 31 décembre de l\'année des 25 ans — soit une naissance en ' + (an - 25) + '.'
      : 'Un Livret Jeune se ferme le 31 décembre de l\'année de vos 25 ans. La date d\'ouverture ne suffit pas à la déduire : on peut en ouvrir un de 12 à 25 ans.';
  } else if (ouv) {
    el.textContent = 'Ce livret n\'a pas de terme réglementaire.';
  } else {
    el.textContent = '';
  }
}
